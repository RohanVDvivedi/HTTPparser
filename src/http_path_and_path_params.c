...
int parse_url_encoded_param(stream* rs, dstring* key, dstring* value, int is_first_param)
{
	...
	dstring key_encoded = read_until_any_end_chars_from_stream(rs, is_end_char_for_param_key, NULL, &last_byte, min(2048, MAX_UNREAD_BYTES_COUNT/2), &stream_error);
	if(stream_error)
		return HTTP_ERROR_IN_STREAM;
	...
	dstring value_encoded = read_until_any_end_chars_from_stream(rs, is_end_char_for_param_value, NULL, &last_byte, min(2048, MAX_UNREAD_BYTES_COUNT/2), &stream_error);
	if(stream_error)
	{
		deinit_dstring(&key_encoded);
		return HTTP_ERROR_IN_STREAM;
...
int parse_http_path_and_path_params(stream* rs, http_request_head* hr_p)
{
	...
	{
		path_and_params = read_until_dstring_from_stream(rs, &SP, SP_spml, min(2048, MAX_UNREAD_BYTES_COUNT/2), &stream_error);
		if(stream_error)
			return HTTP_ERROR_IN_STREAM;
